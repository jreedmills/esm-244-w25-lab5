...
seeds_longer <- seeds_df |>
  pivot_longer(cols = -variety)

seeds_hist <- ggplot(seeds_longer, aes(x = value)) +
  geom_histogram() +
  facet_grid(variety ~ name, scales = 'free') +
  theme_bw() 

seeds_scatter1 <- ggplot(seeds_df, aes(x = a, y = asym)) +
  geom_point(aes(color = variety)) +
  labs(x = "kernel area", y = "asymettry coefficient") +
  theme_bw()

seeds_scatter2 <- ggplot(seeds_df, aes(x = l_g, y = w_k)) +
  geom_point(aes(color = variety)) +
  labs(x = "length of kernel groove", y = "kernel width") +
  theme_bw()
...
```{r}
#| fig-cap: The figure shows that there is a prominent kink at 3 clusters, therefore we will group our data into 3 clusters.
...
seeds_scatter3 <- ggplot(seeds_cl) +
...
```{r}
#| tbl-cap: continency table for the groupings of kmeans and the varieties of kernels
...
1. Slice the top 20 GHG emitters

2. Drop NAs (note: we dont have any in this case)

3. Select only numerics

4. Scale the data

5. Get the distance of the scaled data

6. Run hclust

7. Linkages (single or complete)

8. Make our dendrograms
...
p_combined <- p_complete + p_single + plot_layout(ncol = 2)
p_combined
